...
public class TreeNode<Key: Comparable, T> {
  public typealias Node = TreeNode<Key, T>
  
  var value: T? // Value held by the node
  ...
  public convenience init(key: Key, value: T?) {
    self.init(
      key: key,
      value: value,
      leftChild: nil,
...
// MARK: - The Sparse tree

/// Custom implementation of a AVL Tree.
/// This implementation is well suited for in-memory usage.
///
/// The following optiomizations assumptions have been made.
/// - `Storage`: There are lesser non-nil elements than nil and thus lesser memory can be used to store only those elements.
/// - `Computing time`: Computing time can be saved by logically designing a data structure traversing only non-nil elements.
open class SparseTree<Key: Comparable, T> {
  public typealias Node = TreeNode<Key, T>
...
extension SparseTree {
  subscript(key: Key) -> T? {
    get { return search(input: key) }
    set { insert(key: key, value: newValue) }
  }
  
  public func search(input: Key) -> T? {
...
extension SparseTree {
  public func insert(key: Key, value: T? = nil) {
    if let root = root {
  ...
  private func insert(input: Key, value: T?, node: Node) {
    if input < node.key {
      if let child = node.leftChild {
        insert(input: input, value: value, node: child)
      } else {
        let child = Node(key: input, value: value, leftChild: nil, rightChild: nil, parent: node, height: 1)
...
extension TreeNode: CustomDebugStringConvertible {
  public var debugDescription: String {
    var s = "key: \(key), value: \(String(describing: value)), height: \(height)"
...
extension SparseTree: CustomDebugStringConvertible {
  public var debugDescription: String {
...
extension SparseTree: CustomStringConvertible {
  public var description: String {
